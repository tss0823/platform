...
public class RandomValidateCode {
    ...
    public void getRandcode(HttpServletRequest request, HttpServletResponse response) {
        if (cacheService == null) {
            WebApplicationContext wc = WebApplicationContextUtils.getWebApplicationContext(ServeltContextManager.getServletContext());
        ...
        // 验证码key add by shengshan.tang at 2014-04-28
        String sid = WebUtils.getSessionId(request);
        String validateCodeKey = StrUtils.join(CommConstant.VALIDATE_CODE, sid);
        cacheService.set(validateCodeKey, randomString, DateUnitEnum.MINUTE, 3); // 3分钟过期
        g.dispose();
        try {
